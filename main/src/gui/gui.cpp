...
void GUI::updateWifiNetworkList(int networkCount, struct WLAN::cNetwork* networks) {
  ...
  if (networks != NULL && networkCount > 0) {
    ...
    for(int i = 0; i < networkCount; ++i) {
        ...
        // keeps a copy of a pointer to the network information and its corresponding subpage
        // this is so that the user may select a subpage and attempt to login to the selected network
        nInfoEntry.network = networks[i];
        nInfoEntry.network_sub_page = network_sub_page;
        networkInfo.push_back(nInfoEntry);
        ...
        if(WiFi.status() == WL_CONNECTED && networkCount == 1) {
            ...
            lv_obj_add_event_cb(disconnect_btn, disconnect_network_submenu_handler, LV_EVENT_CLICKED, netData);
            ...
            lv_obj_add_event_cb(cont, network_submenu_handler, LV_EVENT_CLICKED, netData);
        }
    }
  } else {
    lv_obj_t *cont = lv_menu_cont_create(wifi_sub_page);
...
void GUI::setup_start_screen() {
    ...
    lv_obj_add_event_cb(start_btn, start_button_handler, LV_EVENT_ALL, nullptr);
    ...
    lv_obj_add_event_cb(settings_btn, settings_button_handler, LV_EVENT_ALL, nullptr);
...
void GUI::setup_wifi_prompt_screen() {
    ...
    lv_obj_add_style(settings_btn, &nobg_btn_style, 0);
    lv_obj_add_event_cb(settings_btn, settings_button_handler_special, LV_EVENT_ALL, nullptr);
...
void GUI::slider_event_cb(lv_event_t * e)
{
    ...
    SliderInfo* sliderInfo = (SliderInfo *)lv_event_get_user_data(e);
    char buf[16];
...
lv_obj_t* GUI::create_slider(lv_obj_t * parent, const char * icon, const char * txt, int32_t min, int32_t max, int32_t val, lv_font_t * fontSize, int iconScalingFactor)
{
    ...
    SliderInfo* sliderInfo = (SliderInfo*) malloc(sizeof(SliderInfo));
...
void GUI::setup_settings_screen() {
    ...
    lv_obj_add_event_cb(cont, wifi_submenu_handler, LV_EVENT_CLICKED, nullptr);
...
void GUI::setup_screen_template(lv_obj_t * screen, char* title) {
    ...
    lv_obj_add_event_cb(back_btn, default_back_btn_handler, LV_EVENT_CLICKED, nullptr);
...
///
void GUI::set_active_game_state() {
...
void GUI::setup_active_game_screen() {
